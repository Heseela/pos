import { Table, CartItem, SavedBill } from '@/lib/types';
import { ShoppingBag, Printer, ShoppingCart, Save, Percent, Edit2, Trash2 } from 'lucide-react';
...
export default function OrderCart({
  ...
  const handleLoadBill = (bill: SavedBill) => {
    if (window.confirm(`Load saved bill for Table ${bill.tableNumber}? This will replace current cart.`)) {
  ...
  return (
    <div className="bg-white rounded-xl border p-6 sticky top-6 shadow-lg">
      <div className="flex items-center justify-between mb-6">
        ...
        {savedBills.filter(bill => bill.billType === 'table').length > 0 && (
          <button
          ...
          >
            ...
            Saved Bills ({savedBills.filter(bill => bill.billType === 'table').length})
          </button>
        )}
      </div>
            ...
            {filteredBills.map(bill => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-medium">Table {bill.tableNumber}</div>
                    <div className="text-sm text-gray-500">
                      {bill.customerName} • {bill.cart.length} items
                    </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm flex items-center justify-center gap-2"
              >
                <Save className="w-4 h-4" />
                Save Bill
              </button>
